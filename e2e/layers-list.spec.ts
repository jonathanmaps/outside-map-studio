...
describe("layers list", () => {
  ...
  describe("line", () => {
    ...
    test("searches layer metadata with field filters", async () => {
      const lineId = await when.modal.fillLayers({ id: "trail-line", type: "line", layer: "example" });
      await when.modal.open();
      const backgroundId = await when.modal.fillLayers({ id: "paper-background", type: "background" });

      const search = get.elementByTestId("layer-list:search");
      await search.fill("type:line trail");
      await then(get.elementByTestId("layer-list-item:" + lineId)).shouldBeVisible();
      await then(get.elementByTestId("layer-list-item:" + backgroundId)).shouldNotExist();

      await search.press("Escape");
      await then(get.elementByTestId("layer-list-item:" + backgroundId)).shouldBeVisible();
    });

    test("focuses a layer without changing the saved style", async () => {
      const id = await when.modal.fillLayers({ id: "focused-line", type: "line", layer: "example" });
      await when.click("layer-list-item:" + id + ":focus");

      await then(get.elementByTestId("layer-list:clear-isolation")).shouldBeVisible();
      await then(get.styleFromLocalStorage()).shouldDeepNestedInclude({
        layers: [{ id, type: "line", source: "example" }],
      });

      await when.click("layer-list:clear-isolation");
      await then(get.elementByTestId("layer-list:clear-isolation")).shouldNotExist();
    });
  });
