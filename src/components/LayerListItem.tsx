...
import { MdContentCopy, MdVisibility, MdVisibilityOff, MdDelete, MdCenterFocusStrong } from "react-icons/md";
...
class IconAction extends React.Component<IconActionProps> {
  renderIcon() {
    switch (this.props.action) {
      ...
      case "focus": return <MdCenterFocusStrong />;
    }
  }
...
type LayerListItemProps = {
  ...
  onLayerIsolationToggle?(...args: unknown[]): unknown
  isIsolated?: boolean
};
...
export const LayerListItem = React.forwardRef<HTMLLIElement, LayerListItemProps>((props, ref) => {
  ...
  return <IconContext.Provider value={{ size: "14px" }}>
    <li
      ...
      />
      <IconAction
        wdKey={"layer-list-item:" + props.layerId + ":focus"}
        action={"focus"}
        classBlockName="focus"
        classBlockModifier={props.isIsolated ? "active" : undefined}
        onClick={event => {
          event.stopPropagation();
          onLayerIsolationToggle!(props.layerIndex);
        }}
      />
      <IconAction
